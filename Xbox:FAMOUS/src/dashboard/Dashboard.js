...
Dashboard.prototype = Object.create(Node.prototype);

// Overwrite on mount to emit the changeSection event the moment
// twitter is added to the scene graph.
Dashboard.prototype.onMount = function onMount (parent, id) {
...
// the scene graph
Dashboard.prototype.onReceive = function onReceive (event, payload) {
...
function makeFooter (node) {
    ...
    node.addChild()
        .setDifferentialSize(-40, null, null)
        .setPosition(20, 0)
        .setSizeMode('default', 'absolute')
        .setAbsoluteSize(1, 100)
        .setMountPoint(0, 0)
        .setAlign(0, 0.1)
        .addChild(new Footer());
}
